...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages; ni++)
    {
        ...
        im = cv::imread(string(argv[3])+"/color/"+vstrImageFilenames[ni],CV_LOAD_IMAGE_UNCHANGED);
        cv::resize(im, im, cv::Size(640, 480));
        depth = cv::imread(strFile + "/" + vstrDepthFilenames[ni], CV_LOAD_IMAGE_UNCHANGED);
        ...
        cout << vstrImageFilenames[ni] << ' ' << vstrDepthFilenames[ni] << endl;
    ...
    auto trajString = "evaluation/RGBD_Scannet_"+dataset_name+"_KeyFrameTrajectory";
    auto snapString = "evaluation/RGBD_Scannet_"+dataset_name+".msgpack";
    auto gtJsonTrajString = "evaluation/RGBD_Scannet_"+dataset_name+"_gtTraj.json";
